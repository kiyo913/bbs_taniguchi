...
if( !empty($_POST['btn_submit'])){
    ...
    if(empty($error_message)){
        ...
        if($mysqli->connect_errno){
            ...
            // データを登録するSQL作成
            $sql = "INSERT INTO post_taniguchi (name,post_datetime,post_text,parent_id) VALUES ('{$postdata['name']}','{$now_date}','{$postdata['post_text']}',{$id})";
            ...
            if($res){
                ...
                header('Location: reply.php?id='.$id);　//投稿終了時、返信画面へ移動し重複投稿を回避する
                exit;
            }else{
